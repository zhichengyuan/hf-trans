  ...
  <div class="upload-container">
    ...
    <el-dialog :visible.sync="dialogVisible">
      <el-upload :multiple="true" :file-list="fileList" :show-file-list="true" :on-remove="handleRemove"
        :on-success="handleAvatarSuccess" :before-upload="beforeUpload" class="editor-slide-upload"
        :action="uploadImg()" list-type="picture-card">
  ...
  export default {
    ...
    methods: {
      ...
      handleAvatarSuccess(res, file) {
        ...
        //this.$set(this.form, 'img', res.data.fid)
        console.log("res", res, file)
        const uid = file.uid
        // const uid = res.data.fid
        const objKeyArr = Object.keys(this.listObj)
        for (let i = 0, len = objKeyArr.length; i < len; i++) {
          console.log(this.listObj[objKeyArr[i]].uid, uid)
          if (this.listObj[objKeyArr[i]].uid === uid) {
            this.listObj[objKeyArr[i]].url = ImgUrl(res.data.fid)//response.files.file
